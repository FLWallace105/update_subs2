...
module FixSubInfo
  class SubUpdater
    ...
    def write_csv_empty_product_collections
      puts "Starting"
      File.delete('null_product_collection.csv') if File.exist?('null_product_collection.csv')

      
      column_header = ["subscription_id", "email", "next_charge_scheduled_at", "is_prepaid", "product_title", "line_item_properties" ]
      my_sub_sql = "select sub_collection_sizes.subscription_id, subscriptions.email, subscriptions.next_charge_scheduled_at, subscriptions.is_prepaid, subscriptions.product_title, subscriptions.raw_line_item_properties from sub_collection_sizes, subscriptions where subscriptions.subscription_id = sub_collection_sizes.subscription_id and sub_collection_sizes.next_charge_scheduled_at > '2022-07-31' and sub_collection_sizes.next_charge_scheduled_at < '2022-09-01' and sub_collection_sizes.product_collection is null"

      my_subs = ActiveRecord::Base.connection.exec_query(my_sub_sql)

      my_orders_sql = "select order_collection_sizes.order_id, orders.first_name, orders.last_name, orders.email, order_line_items_fixed.title, order_line_items_fixed.product_title, order_line_items_fixed.subscription_id,  order_collection_sizes.scheduled_at, orders.line_items from orders, order_collection_sizes, order_line_items_fixed where orders.order_id = order_collection_sizes.order_id and order_line_items_fixed.order_id = order_collection_sizes.order_id and order_collection_sizes.product_collection is null and order_collection_sizes.scheduled_at > '2022-07-31' and order_collection_sizes.scheduled_at < '2022-09-01' and orders.is_prepaid = '1' "

      my_orders = ActiveRecord::Base.connection.exec_query(my_orders_sql)
      

       CSV.open('null_product_collection.csv','a+', :write_headers=> true, :headers => column_header) do |hdr|
         column_header = nil

          my_subs.each do |mys|
            puts mys.inspect
            
            csv_data_out = [mys['subscription_id'], mys['email'], mys['next_charge_scheduled_at'], mys['is_prepaid'], mys['product_title'], JSON.parse(mys['raw_line_item_properties']) ]
            hdr << csv_data_out
          ...
          hdr << ["order_id", "first_name", "last_name", "email", "title", "product_title", "subscription_id", "scheduled_at", "line_items"]

          my_orders.each do |myo|
            puts myo.inspect
            csv_data_out = [myo['order_id'], myo['first_name'], myo['last_name'], myo['email'], myo['title'], myo['product_title'], myo['subscription_id'], myo['scheduled_at'], JSON.parse(myo['line_items']) ]
            hdr << csv_data_out
    ...
    def setup_subscription_update_table
      ...
      aug2022_monthly_straggler = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2022-07-31' and subscriptions.next_charge_scheduled_at < '2022-09-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and subscriptions.is_prepaid = \'f\' and ( sub_collection_sizes.product_collection not ilike 'ellie%pick%' and sub_collection_sizes.product_collection not ilike 'gray%matter%' and sub_collection_sizes.product_collection not ilike 'midnight%shine%' and sub_collection_sizes.product_collection not ilike 'cutout%this%'  )   "

      aug2022_prepaid_straggler = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2022-07-31' and subscriptions.next_charge_scheduled_at < '2022-09-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and subscriptions.is_prepaid = \'t\' and ( sub_collection_sizes.product_collection not ilike 'ellie%pick%' and sub_collection_sizes.product_collection not ilike 'gray%matter%' and sub_collection_sizes.product_collection not ilike 'midnight%shine%' and sub_collection_sizes.product_collection not ilike 'cutout%this%'  )   "
      ...
      aug2022_nulls_monthly = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at is null and sub_collection_sizes.subscription_id = subscriptions.subscription_id and ( sub_collection_sizes.product_collection not ilike 'test%value%'  ) and subscriptions.is_prepaid = \'f\' "

      aug2022_nulls_prepaid = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at is null and sub_collection_sizes.subscription_id = subscriptions.subscription_id and ( sub_collection_sizes.product_collection not ilike 'test%value%'  ) and subscriptions.is_prepaid = \'t\' "

      july22_nulls_monthly = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at is null and sub_collection_sizes.subscription_id = subscriptions.subscription_id and ( sub_collection_sizes.product_collection not ilike 'test%value%'  ) and subscriptions.is_prepaid = \'f\' "


      april22_nulls_prepaid = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at is null and sub_collection_sizes.subscription_id = subscriptions.subscription_id and ( sub_collection_sizes.product_collection not ilike 'test%value%'  ) and subscriptions.is_prepaid = \'t\' "
      ...
      june2021_prepaid_after_5 = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2021-06-05' and subscriptions.next_charge_scheduled_at < '2021-07-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and   ( sub_collection_sizes.product_collection not ilike 'test%something%' and  sub_collection_sizes.product_collection not ilike 'test2%'  ) and subscriptions.is_prepaid = \'t\' "

      july21_early_prepaid = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2021-06-30' and subscriptions.next_charge_scheduled_at < '2021-07-06' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and   ( sub_collection_sizes.product_collection not ilike 'test%one%'  ) and subscriptions.is_prepaid = \'t\' "

      aug22_prepaid = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2022-07-31' and subscriptions.next_charge_scheduled_at < '2022-09-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and   ( sub_collection_sizes.product_collection  not ilike 'ellie%pick%'  and sub_collection_sizes.product_collection  not ilike 'test%value%' ) and subscriptions.is_prepaid = \'t\' "

      oct21_dulce_monthly = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2021-09-30' and subscriptions.next_charge_scheduled_at < '2021-11-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id and   ( sub_collection_sizes.product_collection  ilike 'dulce%leche%' ) and subscriptions.is_prepaid = \'f\' "

      aug22_monthly_coverage = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2022-08-10' and subscriptions.next_charge_scheduled_at < '2022-09-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id  and subscriptions.is_prepaid = \'f\' and ( sub_collection_sizes.product_collection not ilike 'cutout%this%' and sub_collection_sizes.product_collection not ilike 'midnight%shine%' and sub_collection_sizes.product_collection not ilike '%2%item%' ) and ( sub_collection_sizes.leggings = 'XL' and sub_collection_sizes.tops = 'XL' and sub_collection_sizes.sports_bra = 'XL' ) LIMIT 20"
      ...
      sep21_monthly_reset = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and subscriptions.next_charge_scheduled_at > '2021-09-04' and subscriptions.next_charge_scheduled_at < '2021-10-01' and sub_collection_sizes.subscription_id = subscriptions.subscription_id  and subscriptions.is_prepaid = \'f\' and (sub_collection_sizes.product_collection  ilike 'moon%shine%' ) "

      elliestaging_monthly = "insert into subscriptions_updated (subscription_id, customer_id, updated_at, created_at,  next_charge_scheduled_at, product_title, status, sku, shopify_product_id, shopify_variant_id, raw_line_items) select subscriptions.subscription_id, subscriptions.customer_id, subscriptions.updated_at, subscriptions.created_at, subscriptions.next_charge_scheduled_at, subscriptions.product_title, subscriptions.status, subscriptions.sku, subscriptions.shopify_product_id, subscriptions.shopify_variant_id, subscriptions.raw_line_item_properties from subscriptions, sub_collection_sizes where subscriptions.status = 'ACTIVE' and  sub_collection_sizes.subscription_id = subscriptions.subscription_id  and subscriptions.is_prepaid = \'f\'  and sub_collection_sizes.product_collection not ilike '%winged%beauty%' "
     ...
     #Now reset index
     ActiveRecord::Base.connection.reset_pk_sequence!('subscriptions_updated')
     ActiveRecord::Base.connection.execute(aug2022_monthly_straggler)
    ...
    def sendgrid_csv
      puts "starting"
      File.delete('sendgrid_emails_sent_skip.csv') if File.exist?('sendgrid_emails_sent_skip.csv')
      sendgrid_file = File.open('sendgrid_emails_sent_skip.csv', 'w')
      sendgrid_file.write("email\n")
      temp_array = Array.new
      CSV.foreach('sendgrid_skip.csv', :encoding => 'ISO-8859-1', :headers => true) do |row|
        #puts row.inspect
        #puts row['subject']
        my_subject = row['subject']
        if my_subject == 'Confirmation of Skipping Your Ellie Subscription'
          #puts row.inspect
          my_email = row['email']
          temp_array.push(my_email)
          #sendgrid_file.write("#{my_email}\n")
        end
        
      end
      puts "Before uniquing elements are -- #{temp_array.count}"
      new_array = temp_array.uniq!
      puts "After uniquing elements are -- #{new_array.count}"
      new_array.each do |mya|
        sendgrid_file.write("#{mya}\n")
    ...
    def recharge_update_monthly_nulls
      puts "Starting ..."
      my_subs = SubscriptionsUpdated.where(updated: false)
      my_subs.each do |sub|
        puts sub.inspect
        my_sub_id = sub.subscription_id
        my_old_sub_info = MonthlySubNull.find_by_subscription_id(my_sub_id)
        my_old_product_id = my_old_sub_info.old_product_id
        my_config_info = MonthlySubNullConfig.find_by_old_product_id(my_old_product_id)
        puts my_config_info.inspect
        my_properties = sub.raw_line_items
        puts "-------------"
        ...
        my_properties.map do |mystuff|
          ...
          if mystuff['name'] == 'product_collection'
            mystuff['value'] = my_config_info.old_product_collection
            
          end
        end
        puts "Changed properties:"
        puts my_properties.inspect

        temp_body = { "sku" => my_config_info.old_sku, "product_title" => my_config_info.old_product_title, "shopify_product_id" => my_config_info.old_product_id, "shopify_variant_id" => my_config_info.old_variant_id, "properties" => my_properties }
        puts "+++++++++++++++++++++++++++"

        puts temp_body.inspect

        body = temp_body.to_json
    ...
    def load_current_products
      ...
      # @conn.prepare('statement1', "#{my_insert}")
      # CSV.foreach('update_marbled_moves_monthly.csv', :encoding => 'ISO-8859-1', :headers => true) do |row|
      #    puts row.inspect
